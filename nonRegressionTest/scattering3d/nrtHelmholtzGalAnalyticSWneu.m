...
% Incident wave
PW = @(X) exp(1i*k*X*(X0'./norm(X0))); % Plane wave
...
SW = @(X) exp(1i*k*Norme(X,X0))./Norme(X,X0); % Spher wave
gradxSW{1} = @(X) (X(:,1)-X0(1)) .* ( 1i*k*Norme(X,X0)-1 ) ./ (Norme(X,X0).^2) .* SW(X);
gradxSW{2} = @(X) (X(:,2)-X0(2)) .* ( 1i*k*Norme(X,X0)-1 ) ./ (Norme(X,X0).^2) .* SW(X);
gradxSW{3} = @(X) (X(:,3)-X0(3)) .* ( 1i*k*Norme(X,X0)-1 ) ./ (Norme(X,X0).^2) .* SW(X);
...
elseif strcmp(type,"spher")
    ...
    error('nrtHelmholtzGalAnalyticSWneu.m : unavailable case')
end
...
Id = integral(sigma,u,v); % domIntegral3
...
RHS = - integral(sigma,ntimes(u),gradxIW); % domIntegral3
...
Sinf = integral(nu,sigma,Ginf,v); % domIntegral4
Dinf = integral(nu,sigma,gradxGinf,ntimes(v)); % domIntegral4
